package io.ideploy.deployment.admin.annotation.log;

import io.ideploy.deployment.admin.context.AdminContext;
import io.ideploy.deployment.admin.utils.SpringContextUtil;
...
public class AdminLoggerAspect {
	...
	@AfterReturning(value = "within(io.ideploy.deployment.admin..*) && @annotation(adminLog)", returning = "obj")
	public void addLogSuccess(JoinPoint jp, AdminLog adminLog, Object obj) {

		logger.info("记录方法执行成功的日志 | signature: {}, targetClass: {}",
				jp.getSignature().toString(), jp.getTarget().getClass().getName());
		try {
	...
	private void processLog(JoinPoint jp, AdminLog adminLog, Object result) throws Exception {
		...
		if (doLog(adminLog, result)) {
			...
			logger.info("参数列表 | admin: {}, type: {}, arguments: {}, result: {}",
					AdminContext.getAccountId(), type, arguments, resultMessage);
			// save log
	...
	private String getArguments(JoinPoint jp, Method method, AdminLog adminLog) throws Exception {
		...
		for (Map.Entry<String, Object> entry : params.entrySet()) {
			...
			if ((value instanceof HttpServletRequest) || (value instanceof HttpServletResponse)
					|| (notNullName.indexOf("MODEL_KEY_PREFIX") >= 0) || notNullName.indexOf("_AUTH") > 0) {
				...
				if (lowerName.indexOf("password") >= 0 || lowerName.indexOf("appkey") >= 0
						|| lowerName.indexOf("platformkey") >= 0) {
					// 不输出密码之类的信息
					value = "***";
				...
				// 保存json信息
				buf.append(name).append("=").append(JsonUtil.toJSONString(value));
			}
	...
	private String getResultMessage(AdminLog adminLog, Object result) {
		...
		if (result != null) {
			if (result instanceof ModelAndView) {
				...
				if ("/error".equals(view.getViewName()) || "/exception".equals(view.getViewName())) {
				...
				if ("/error".equals(view) || "/exception".equals(view)) {
	 ...
	 */
	public static Map<String, Object> getJointPointParam(JoinPoint jp)
	 ...
	 * @throws IllegalArgumentException
	 */
	public static Map<String, Object> getClassAttrValueMap(Class<?> clazz, String name, Object value)
			throws IllegalArgumentException, IllegalAccessException {
		...
		for (Field field : fields) {
			...
			// 暂不递归获取
			// else {
			// Map<String, Object> attr = getClassAttrValueMap(_clazz, _name,
			// _value);
			// attrs.putAll(attr);
			// }
		}
	...
	@AfterThrowing(pointcut = "within(io.ideploy.deployment.admin..*) && @annotation(adminLog)", throwing = "ex")
	public void addLogException(JoinPoint jp, AdminLog adminLog, Exception ex) {
		logger.error("记录方法执行异常的日志 | signature: {}, targetClass: {}, message: {}",
				jp.getSignature().toString(), jp.getTarget().getClass().getName(), ex.getMessage());
	}
}
