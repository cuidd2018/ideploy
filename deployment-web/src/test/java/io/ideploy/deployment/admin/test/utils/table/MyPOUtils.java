...
public class MyPOUtils {
	 ...
	 */
	public MyPOUtils(Connection conn, String tablename, String basePackage) {
		this.conn = conn;
	...
	public void writeMyBatisFile(String outputPath, String keyProperty, boolean useGeneratedKeys) throws Exception {
		...
		File template = new File(MyPOUtils.class.getResource("").getFile() + "mybatis_template.xml");
...
//		content = StringUtils.replace(content, "${listAllSql}", listAllSql);
	...
	public void writeDaoFile(String outputPath, String author) throws Exception {
		...
		// 生成 DAO 接口文件
		File daoTemplateFile = new File(MyPOUtils.class.getResource("").getFile() + "java_dao.template");
		...
		// 生成 DaoImpl 文件
		if (daoImpl) {
			File daoImplTemplateFile = new File(MyPOUtils.class.getResource("").getFile() + "java_dao_impl.template");
...
//	}
	...
	public void processUpdateSQL(StringBuilder sb) {
		...
		for (String key : sqlKeyNames) {
			if (1 < index++)
				sb.append(" and ");
			sb.append(key).append("=#{").append(splitTable ? "po." : "").append(getJavaName(key)).append("}");
		}
	}
	...
	public static void run(boolean genDaoImpl, String un, String ps, String url, String pack, String user, String path_, boolean autoIdl, String... tables) {
		...
		try {
			...
			for (String table : tables) {
				MyPOUtils t = new MyPOUtils(con, table, pack);
