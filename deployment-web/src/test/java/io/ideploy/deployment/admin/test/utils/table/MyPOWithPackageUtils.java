package io.ideploy.deployment.admin.test.utils.table;
 ...
 * 3 各位自己可以修改此工具用到项目中去.
 */
public class MyPOWithPackageUtils {

	private final String mp = "_mp";// 表名以此结尾的，不生成DTO、Service文件
	private boolean daoImpl = false;// 是否生成DAO实现类，false则生成package-info来代替
	private boolean poToString = false;// PO对象是否生成 toString 方法
	private final String modelName = "po";

	private String modualPackage = "";//模块名称
	 ...
	 * 
	 * @param conn - 数据库连接
	 * @param basePackage - 基于哪个package生成代码，例如: io.ideploy.commons
	 */
	public MyPOWithPackageUtils(Connection conn, String tablename, String basePackage, String modualPackage) {
		this.conn = conn;
		this.tablename = tablename;
		this.basePackage = basePackage;
		this.modualPackage = modualPackage;
	...
	public void writeMyBatisFile(String outputPath, String keyProperty, boolean useGeneratedKeys) throws Exception {
		String fileName = outputPath + File.separator + "sql" + File.separator + modualPackage + File.separator + tablename + ".xml";
		File template = new File(MyPOWithPackageUtils.class.getResource("").getFile() + "mybatis_template.xml");

		String content = FileUtils.readFileToString(template, "utf-8");
		String namespace = basePackage + ".dao." + modualPackage + "." + getJavaName(initcap(tablename)) + "Dao";
		String poName = basePackage + "." + modelName + "." + modualPackage + "." + getJavaName(initcap(tablename)) + "PO";
...
//		content = StringUtils.replace(content, "${listAllSql}", listAllSql);
		...
		doWriteToFile(content, fileName);
	...
	private void writePO(String path, String content) throws Exception {
		...
		String fileName = path + File.separator + modelName + File.separator + modualPackage + File.separator + classBaseName + "PO.java";
		doWriteToFile(content, fileName);
	}
	...
	private void writeDTO(String path, String content) throws Exception {
		if (tablename.endsWith(mp))
			return;
		String classBaseName = getJavaName(initcap(tablename));
		String fileName = path + File.separator + "vo" + File.separator + modualPackage + File.separator + classBaseName + ".java";
		doWriteToFile(content, fileName);
	}
	 ...
	 * @throws IOException
	 */
	protected static ArrayList<String> parentDirectory = new ArrayList<String>();
	protected void doWriteToFile(String content, String fileName) throws IOException {
		
		File file = new File(fileName);
		File parentFile = file.getParentFile();
		String parentFullPath = parentFile.getCanonicalPath();
		//删过一次的就不用再删
		if(!parentDirectory.contains(parentFullPath)){
			parentDirectory.add(parentFullPath);
			if(parentFile.exists() && parentFile.isDirectory()){
				//System.err.println("       > 删除目录: " + parentFile);
				deleteAllFilesOfDirectory(parentFile);
			}
		}
	...
	/**
	 * 递归删除这个目录下的所有文件以及子目录的所有文件
	 * @param path
	 */
	public static void deleteAllFilesOfDirectory(File path) {
		if (!path.exists())
			return;
		if (path.isFile()) {
			path.delete();
		...
		for (int i = 0; i < files.length; i++) {
			deleteAllFilesOfDirectory(files[i]);
		}
		path.delete();
	}
	...
	public void writeDaoFile(String outputPath, String author) throws Exception {
		String opDao = outputPath + File.separator + "dao" + File.separator + modualPackage;
		String classBaseName = getJavaName(initcap(tablename));
		String daoPackage = basePackage + ".dao" + "." + modualPackage;
		String implPackage = basePackage + ".dao" + "." + modualPackage + ".impl";
		...
		String model = basePackage + "." + modelName + "." + modualPackage + "." + po;
		...
		// 生成 DAO 接口文件
		File daoTemplateFile = new File(MyPOWithPackageUtils.class.getResource("").getFile() + "java_dao.template");
		...
		String imTheDao = "";
		if (!(daoImpl)) {
			daoContent = StringUtils.replace(daoContent, "${theDao}", "@MyBatisDao");
			imTheDao = lineSeperator + "import io.ideploy.deployment.datasource.MyBatisDao;" + lineSeperator;
		}
		daoContent = StringUtils.replace(daoContent, "${imTheDao}", imTheDao);
		String daoFileName = opDao + File.separator + daoClass + ".java";
		doWriteToFile(daoContent, daoFileName);
		...
		// 生成 DaoImpl 文件
		if (daoImpl) {
			File daoImplTemplateFile = new File(MyPOWithPackageUtils.class.getResource("").getFile() + "java_dao_impl.template");
			...
			doWriteToFile(implContent, daoImplFileName);
		}
...
//				sb.append(" * ").append(lineSeperator);
//				sb.append(" */").append(lineSeperator);
//				sb.append("@com.xx.commons.datasource.annocation.TheTypeAlias").append(lineSeperator);
...
//		String po = classBaseName + "PO";
//		String voModel = basePackage + ".vo." + classBaseName;
...
//		daoContent = StringUtils.replace(daoContent, "${voModel}", voModel);
//		daoContent = StringUtils.replace(daoContent, "${model}", model);
//		daoContent = StringUtils.replace(daoContent, "${author}", author);
//		daoContent = StringUtils.replace(daoContent, "${table}", tablename);
//		daoContent = StringUtils.replace(daoContent, "${date}", today);
...
//		daoContent = StringUtils.replace(daoContent, "${pksStr}", pksStr.toString());
//		daoContent = StringUtils.replace(daoContent, "${vo}", classBaseName);
//		daoContent = StringUtils.replace(daoContent, "${po}", po);
...
//		String daoImplTemplate = FileUtils.readFileToString(daoImplTemplateFile, "utf-8");
//		String implContent = StringUtils.replace(daoImplTemplate, "${package}", implPackage);
//		implContent = StringUtils.replace(implContent, "${voModel}", voModel);
//		implContent = StringUtils.replace(implContent, "${model}", model);
//		implContent = StringUtils.replace(implContent, "${author}", author);
//		implContent = StringUtils.replace(implContent, "${table}", tablename);
//		implContent = StringUtils.replace(implContent, "${date}", today);
...
//		implContent = StringUtils.replace(implContent, "${vo}", classBaseName);
//		implContent = StringUtils.replace(implContent, "${po}", po);
...
//	}
	...
	private String parse(String[] colNames, String[] colTypes, int[] colSizes) {
		...
		if (StringUtils.isNotEmpty(basePackage)) {
			if(StringUtils.isBlank(modualPackage)){
				sb.append("package ").append(basePackage).append(".").append(modelName).append(";").append(lineSeperator).append(lineSeperator);
			}else{
				sb.append("package ").append(basePackage).append(".").append(modelName).append(".").append(modualPackage).append(";").append(lineSeperator).append(lineSeperator);
			}
		}
	...
	private String parseDto(String[] colNames, String[] colTypes, int[] colSizes) {
		...
		if (StringUtils.isNotEmpty(basePackage)){
			if(StringUtils.isBlank(modualPackage)){
				sb.append("package ").append(basePackage).append(".vo;").append(lineSeperator).append(lineSeperator);
			}else{
				sb.append("package ").append(basePackage).append(".vo.").append(modualPackage).append(";").append(lineSeperator).append(lineSeperator);
			}
		}
		sb.append("import java.io.Serializable;").append(lineSeperator);
		if (importUtil)
	...
	public void processInsertSQL(StringBuilder sb) {
		...
		for (int i = 0; i < colnames.length; i++) {
			...
			sb.append("#{");
			if (splitTable) {
				sb.append("po.").append(modualPackage + ".");
			}
	...
	public void processUpdateSQL(StringBuilder sb) {
		...
		for (String key : sqlKeyNames) {
			if (1 < index++)
				sb.append(" and ");
			sb.append(key).append("=#{").append(splitTable ? (StringUtils.isBlank(modualPackage) ? "po." : ("po." + modualPackage + ".")) : "").append(getJavaName(key)).append("}");
		}
	}
...
//	}
	...
	protected String processGetByIdSql() {
		...
		int index = 1;
		for (String key : sqlKeyNames) {
			String name = getJavaName(key);// 如果不生成daoImpl，那么参数名为：param1 param2 ..
			sql.append(1 < index++ ? " AND " : "").append(key).append("=#{").append(name).append("}");
		}
		return sql.toString();
	}
	 ...
	 * @param user 作者
	 * @param path_ 写入文件路径
	 * @param autoIdl 主键是否自增
	 * @param tables 生成的表
	 */
	public static void run(String un, String ps, String url, String pack, String modualPackage, String user, String path_, boolean autoIdl, String... tables) {
		run(false, un, ps, url, pack, modualPackage, user, path_, autoIdl, tables);
	}
	
	public static void run(boolean genDaoImpl, String un, String ps, String url, String pack, String modualPackage, String user, String path_, boolean autoIdl, String... tables) {
		Connection con = null;
		try {
			...
			for (String table : tables) {
				MyPOWithPackageUtils t = new MyPOWithPackageUtils(con, table, pack, modualPackage);
...
//				t.writeServiceFile(path, user);
		...
		System.out.println(pack + "  ( modual: " + modualPackage + " )" + "  Done!");
	}
