package io.ideploy.deployment.common.util.redis;

import io.ideploy.deployment.common.util.redis.impl.RedisClusterImpl;
import io.ideploy.deployment.common.util.redis.impl.RedisImpl;
...
public class RedisFactory extends JedisPoolConfig {
	...
	/**
	 * 初始化 io.ideploy.platform.commons.cache.redis.Redis 并得到一个实例
	 * 
	...
	private LinkedList<JedisShardInfo> splitServerConfig(String server) {
		...
		for (String tmp : servers) {
			...
			if (StringUtils.isNotBlank(auth)) { //密码不为空时候才设置密码，兼容redis不需要密码的场景
